...
/**
 * The customer form, shared by the create and edit screens.
 *
 ...
 *  - customers keep a field-level change log, so the save records what
...
class CustomerForm extends Form
{
    ...
    public function setCustomer(Customer $customer): void
    {
        ...
        $this->state_province = (string) $customer->state_province;
        $this->city = (string) $customer->city;
        $this->suburb_barangay = (string) $customer->suburb_barangay;
        ...
        $this->payment_terms = (string) $customer->payment_terms;
        $this->credit_amount = (string) Money::round($customer->credit_amount);
        $this->status = $customer->status->value;

        $contacts = collect($customer->contact_persons ?? [])
    ...
    /**
     * Create or update the customer and record the change in its log.
     */
    public function save(): Customer
    {
        ...
        if ($this->customer === null) {
            ...
            $customer->logCreated($this->nullable($data['update_remarks'] ?? null));

            return $customer;
        }

        // Snapshot before the write so the log records the real diff.
        $before = $this->customer->only(self::ATTRIBUTES);

        $this->customer->update($attributes);
        $this->customer->logUpdated($before, $attributes, $this->nullable($data['update_remarks'] ?? null));
