...
<div class="space-y-4">
    <x-page-header :title="$this->isEditing() ? 'Edit ' . $purchaseOrder->code : 'New Purchase Order'"
    ...
    <form wire:submit="save" class="space-y-4">
        ...
        <x-card title="Order details">
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="lg:col-span-2">
                    <x-form.field label="Vendor" for="vendor_id" :error="$errors->first('form.vendor_id')" required>
                        <select id="vendor_id"
                                class="select select-bordered w-full @error('form.vendor_id') select-error @enderror"
                    ...
                    @if ($vendor)
                        <p class="mt-1 text-xs text-base-content/60">
                            Terms: {{ $vendor->payment_terms ?: 'not set' }}
                            · Credit limit {{ $currency }} {{ number_format((float) $vendor->credit_amount, 2) }}
                        </p>
                    @endif
                </div>
                ...
                <x-form.field label="Delivery date" for="delivery_date" :error="$errors->first('form.delivery_date')"
                              hint="Leave blank if the vendor has not committed.">
                ...
                <x-form.field label="Reference" for="reference_no" :error="$errors->first('form.reference_no')"
                              hint="Your purchase requisition or the vendor's quote number.">
                ...
                <div class="lg:col-span-3">
                    <x-form.field label="Remarks" for="remarks" :error="$errors->first('form.remarks')">
                        <input id="remarks" type="text"
                               ...
                               placeholder="Anything the warehouse or the vendor should know"
                               maxlength="2000"
                    ...
                    <div class="rounded-box border border-base-300 p-3" wire:key="item-{{ $index }}">
                        <div class="grid gap-3 lg:grid-cols-12">
                            ...
                            <div class="lg:col-span-2">
                                ...
                                @if ($received > 0)
                                    <p class="mt-1 text-xs text-base-content/60">
                                        {{ number_format($received, 2) }} already received
                                    </p>
                                @endif
                            </div>
                            ...
                            <div class="lg:col-span-2">
                                <x-form.field label="Unit cost"
                                              :error="$errors->first('form.items.' . $index . '.unit_cost')">
                                    <label class="input input-bordered flex w-full items-center gap-1 @error('form.items.' . $index . '.unit_cost') input-error @enderror">
                                        <span class="text-xs text-base-content/60">{{ $currency }}</span>
                                        <input type="number" step="0.01" min="0"
                                               class="tabular grow text-right"
                                               wire:model.live.debounce.400ms="form.items.{{ $index }}.unit_cost">
                                    </label>
                                </x-form.field>
                            </div>
        ...
        <x-card>
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('purchase-orders.index') }}" class="btn btn-ghost btn-sm ml-auto" wire:navigate>Cancel</a>
            </div>
        </x-card>
    </form>
</div>
