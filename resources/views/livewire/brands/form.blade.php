<div class="mx-auto max-w-2xl space-y-4">
    <x-page-header :title="$this->isEditing() ? 'Edit ' . $brand->name : 'Create Brand'"
                   :subtitle="$this->isEditing() ? 'Update this brand' : 'Add a manufacturer or label'">
        <x-slot:actions>
            <a href="{{ route('brands.index') }}" class="btn btn-ghost btn-sm" wire:navigate>
        ...
        <x-card>
            ...
            <x-slot:footer>
                <button type="submit" class="btn btn-primary btn-sm" wire:loading.attr="disabled">
                    ...
                    {{ $this->isEditing() ? 'Save changes' : 'Create Brand' }}
                </button>
                ...
                <a href="{{ route('brands.index') }}" class="btn btn-ghost btn-sm ml-auto" wire:navigate>Cancel</a>
            </x-slot:footer>
        </x-card>
    </form>
</div>
