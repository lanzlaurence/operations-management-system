@php
    ...
    $prompts = [
        ...
        'revert' => [
            'Revert this issue to pending?',
            'It becomes editable again and its quantities are held against the order once more. No stock moves.',
            'Revert to pending',
            'btn-warning',
        ],
        'delete' => [
            'Delete this goods issue?',
            'The pending issue is removed and the quantities it held are released back to the order.',
            'Delete issue',
            'btn-error',
        ],
    ];
...
<div class="space-y-4">
    <x-page-header :title="$issue->code"
                   :subtitle="$issue->salesOrder?->code . ' · ' . $issue->salesOrder?->customer?->name">
        <x-slot:actions>
            <span class="badge {{ $statusBadge[$issue->status->value] ?? 'badge-ghost' }}">
                {{ $issue->status->label() }}
            </span>
            ...
            @if ($actions['edit'])
                @can('goods-issue-edit')
                    <a href="{{ route('goods-issues.edit', $issue) }}" class="btn btn-ghost btn-sm" wire:navigate>
    ...
    @if ($issue->status->isPending())
        <div class="alert alert-warning alert-soft">
            <x-icon name="clock" class="size-5" />
            <span>
                This issue is prepared but not completed, so no stock has moved yet. Its quantities are held
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-box border border-base-300 bg-base-100 p-4">
            <p class="text-sm text-base-content/60">Sales order</p>
            ...
            <p class="text-xs text-base-content/50">{{ $issue->salesOrder?->customer?->name }}</p>
        </div>

        <div class="rounded-box border border-base-300 bg-base-100 p-4">
            <p class="text-sm text-base-content/60">Location</p>
            <p class="font-medium">{{ $issue->location?->name }}</p>
            <p class="font-mono text-xs text-base-content/50">{{ $issue->location?->code }}</p>
        </div>
        ...
        <div class="rounded-box border border-base-300 bg-base-100 p-4">
            ...
            <p class="text-xs text-base-content/50">
                {{ $issue->gi_date?->format('M d, Y') }} · {{ $issue->user?->name }}
            </p>
        </div>
    </div>
    ...
    <x-card title="Items">
            ...
            <table class="table table-zebra table-sm">
                <thead>
                    <tr>
                        ...
                        <th class="text-right">Shipped before</th>
                        <th class="text-right">This issue</th>
                    ...
                    @foreach ($issue->items as $item)
                        <tr>
                            ...
                            <td class="tabular text-right text-base-content/60">
                                {{ number_format((float) $item->qty_shipped, 2) }}
                            </td>
                            <td class="tabular text-right font-medium text-success">
                                +{{ number_format((float) $item->qty_to_ship, 2) }}
                            </td>
                            <td class="tabular text-right">{{ number_format((float) $item->unit_price, 2) }}</td>
        ...
        @if ($issue->remarks)
            <div class="mt-4 rounded-box bg-base-200 p-3 text-sm">
                ...
                <p class="mt-1">{{ $issue->remarks }}</p>
            </div>
        @endif
    </x-card>

    {{-- The movements this issue produced --}}
    @if ($movements->isNotEmpty())
        <x-card title="Stock movements" subtitle="What this issue did to inventory">
            <div class="overflow-x-auto">
                <table class="table table-zebra table-sm">
                    <thead>
                        <tr>
    ...
    {{-- Audit trail --}}
    <x-card title="History" :subtitle="$issue->logs->count() . ' entries'">
        <ol class="space-y-2">
