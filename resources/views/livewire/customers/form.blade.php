...
<div class="mx-auto max-w-4xl space-y-4">
    <x-page-header :title="$this->isEditing() ? 'Edit ' . $customer->code : 'Create Customer'"
                   :subtitle="$this->isEditing()
                        ? $customer->name
                        : 'The code is assigned automatically when the customer is saved'">
        <x-slot:actions>
            @if ($this->isEditing())
                <a href="{{ route('customers.show', $customer) }}" class="btn btn-ghost btn-sm" wire:navigate>
    ...
    <form wire:submit="save" class="space-y-4">
        ...
        <x-card title="Basic information">
            <div class="grid gap-4 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    <x-form.field label="Customer name" for="name" :error="$errors->first('form.name')" required>
                        <input id="name"
                               type="text"
                ...
                <x-form.field label="Credit limit" for="credit_amount" :error="$errors->first('form.credit_amount')" required
                              hint="Use 0 when the customer trades on cash terms.">
                ...
                <x-form.field label="Status" for="status" :error="$errors->first('form.status')" required
                              hint="Only active customers can be selected on new sales orders.">
                    <select id="status"
        ...
        <x-card>
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('customers.index') }}" class="btn btn-ghost btn-sm ml-auto" wire:navigate>Cancel</a>
            </div>
        </x-card>
    </form>
</div>
